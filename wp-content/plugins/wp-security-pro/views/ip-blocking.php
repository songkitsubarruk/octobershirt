			...
			<form name="f" method="post" action="" id="iplookup">
				...
				<table style="width:100%;">
					<tr>
						...
						<td style="padding:0px 10px"><input class="mo_wpns_table_textbox" '.$disabled.' id="lookupip" type="text" name="lookupip"
							required placeholder="IP address" value=""  pattern="((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}" /></td>
						<td><input type="submit" '.$disabled.' id="lookup-button" class="button button-primary button-large" value="Lookup IP" /></td>
					</tr>
...
echo'	<div class="mo_wpns_small_layout">';
...
echo'		<h2>Manual Block IP\'s</h2>
			<form name="f" method="post" action="" id="manualblockipform" >
				...
				<td style="padding:0px 10px"><input class="mo_wpns_table_textbox" type="text" name="ip"
					required placeholder="IP address" value="" '.$disabled.' pattern="((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}" /></td>
				<td><input type="submit" '.$disabled.' class="button button-primary button-large" value="Manual Block IP" /></td></tr></table>
			</form>
			<h2>Blocked IP\'s</h2>
...
echo'			</tbody>
		...
		<div class="mo_wpns_small_layout">
			...
			<form name="f" method="post" action="" id="whitelistipform">
				...
				<td style="padding:0px 10px"><input '.$disabled.' class="mo_wpns_table_textbox" type="text" name="ip"
					required placeholder="IP address" value=""  pattern="((^|\.)((25[0-5])|(2[0-4]\d)|(1\d\d)|([1-9]?\d))){4}" /></td>
				<td><input type="submit" '.$disabled.' class="button button-primary button-large" value="Whitelist IP" /></td></tr></table>
			</form>
			...
			<table id="whitelistedips_table" class="display">
				...
				<tbody>';
					...
					{
						echo "<tr><td>".$whitelisted_ip->ip_address."</td><td>".date("M j, Y, g:i:s a",$whitelisted_ip->created_timestamp)."</td><td><a ".$disabled." onclick=removefromwhitelist('".$whitelisted_ip->id."')>Remove</a></td></tr>";
					} 
